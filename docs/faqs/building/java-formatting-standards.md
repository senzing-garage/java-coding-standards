...
- Conditional source-preserve for argument lists — reserved for cases where reflowing would be wrong rather than merely different: an argument list carrying interleaved comments, and CSOFF/CSON regions. 0.7.0 removed the earlier width-based trigger ("the authored first line still fits, so keep the layout"), which made the output depend on the previous pass.
- Text blocks at any indent level — the closing `"""` moves to +4 from the introducing statement and the content lines move with it, so the rendered string is unchanged. Where a leftward move would push a line past column 0 the whole block moves less far instead, leaving the delimiter short of +4 rather than altering the string.
...
- **Spec C6 paren-aligned operator continuation** (binary + ternary, added in 0.4.3; extended to control-flow in 0.5.0) — when an expression is wrapped in any `parenthesized_expression` node, operator continuation aligns under the column immediately after the `(`. `_emit_parenthesized_expression` sets `emitter.paren_align_col` for every parenthesized_expression — both developer-authored grouping parens AND the syntactically-required parens of control-flow constructs (`if (cond)`, `while (cond)`, `for (...)`, `catch (...)`, `synchronized (...)`, `switch (...)`). 0.4.3's earlier behavior restricted this to grouping parens via a denylist (`_PAREN_NOT_GROUPING_PARENT_TYPES`); 0.5.0 removed the denylist after empirical consumer trial showed the extension produced consistent, readable output. `_emit_binary_expression` and `_emit_ternary_expression` each add a paren-aligned wrap candidate tried before standard P2/T2. The yields-to-source-preserve inversion check (added in 0.4.3) continues to apply at every paren level.

- **Paren-alignment yields to source-preservation on inversion** (`_emit_parenthesized_expression` + `_inner_would_invert_paren_align`) — paren-alignment is silently declined for a grouping paren when the inner expression contains an `argument_list` that would source-preserve at a column LESS than the proposed paren-align column. The detection walks the inner subtree, consults `_arg_list_takes_source_preserve_path` to confirm each candidate arg list will actually source-preserve, and only then compares the arg list's continuation columns against the proposed paren-align column. When an inversion is detected, the level reverts to the cumulative `+4` continuation (pre-Bug-3 behavior) so the source-preserved inner content nests correctly inside the outer operator chain. Since 0.7.0 the predicate is column-insensitive — it fires only on interleaved comments and CSOFF regions — so the walk passes it no column at all.

- **Formatter advisory channel for un-fixable source-preserve quirks** (`FormatterWarning` + `Emitter.warnings` + `format_source(source, warnings_out=...)` parameter, 0.4.3+) — there's a category of layout quirk the formatter genuinely cannot fix on its own. The headline case: a long error message wrapped in `throw new IOException("…long literal…" + variable)` where the author manually placed the literal at a low column (e.g. col 14) to make it fit within 80 chars. Each subsequent reformat sees the multi-row arg list and source-preserves it verbatim, even when the surrounding context (`throw` keyword, enclosing `if` bodies) has shifted to a deeper indent. The result is a 80-char-compliant but visually inverted layout — the contained literal sits LESS indented than the statement that owns it. The only way to fix this is to split the literal into smaller concatenated chunks, which is a code change and out of scope for an AST-preserving formatter. The formatter now collects these as `FormatterWarning` records during emit and the CLIs print them to stderr in `path:line:col: WARNING: …` format so developers know which spots warrant manual cleanup. An advisory means the line does NOT fit: `_fire_wrap_overflow_advisory` returns early whenever the widest line is within 80 columns, so nothing is reported unless something overflows. `mvn -Pcheckstyle validate` will therefore fail on an advisory, and the warning marks a spot needing manual cleanup — splitting the literal, extracting a local, or restructuring. (Lines matching checkstyle's own `ignorePattern`, such as `package` and `import`, are excluded from the width accounting and so never raise one.)
